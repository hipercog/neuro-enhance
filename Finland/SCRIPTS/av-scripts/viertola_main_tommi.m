...
for s=1:size(subjects,2)
    ...
    bvfile = ['Kh' subjects{s} '_AVO.vhdr'];
    if exist([datapath '/' bvfile],'file') ~= 2
        bvfile = ['Kh' subjects{s} '_AVV.vhdr'];
    end
    logfile = ['Kh' subjects{s} '-AV.log'];
    eventpath = [savepath '\' subjects{s} '-av-recoded.evt'];
    ...
    if allclear == 1
        ...
        eventpath_notOK = [savepath '\' subjects{s} '-av-recoded_missingTriggers.evt'];
